...
class CyberKillChainClient:
    ...
    def identify_stage_from_iocs(cls, iocs: list[dict]) -> dict:
        ...
        for ioc in iocs:
            ...
            for stage in stages:
                ...
                stage_iocs[stage].append({"type": ioc_type, "value": ioc_value})
        ...
        # Sort by confidence
        sorted_stages = sorted(stage_analysis.items(), key=lambda x: x[1]["confidence"], reverse=True)
    ...
    def get_adjacent_stages(cls, current_stage: KillChainStage) -> dict[str, KillChainStage | None]:
        ...
        return {"previous": previous_stage, "next": next_stage, "current": current_stage}

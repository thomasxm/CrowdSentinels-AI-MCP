...
class SchemaAwareQueryBuilder:
    ...
    def __init__(self, schema: LogSourceSchema, index: str, max_results: int = 100):
    ...
    def build_process_bounds_query(self, process_name: str) -> QueryResult:
        ...
        if not process_field or not event_code:
            raise ValueError(f"Schema '{self.schema.schema_id}' does not support process_create")
...
| LIMIT 1'''
    ...
    def build_process_terminate_query(
        self, process_name: str, host: str | None = None, start_time: str | None = None, end_time: str | None = None
    ) -> QueryResult:
        ...
        if not process_field or not event_code:
            raise ValueError(f"Schema '{self.schema.schema_id}' does not support process_terminate")

        # Build WHERE clause
        conditions = [f'{event_code_field} == "{event_code}"', f'{process_field} LIKE "*{process_name}*"']
...
| LIMIT {self.max_results}"""
    ...
    def build_child_processes_query(
        self, parent_process_name: str, host: str, start_time: str, end_time: str
    ) -> QueryResult:
        ...
        if not parent_field or not event_code:
            raise ValueError(f"Schema '{self.schema.schema_id}' does not support parent process tracking")
...
| LIMIT {self.max_results}'''
    ...
    def build_network_connections_query(
        self, process_name: str, host: str, start_time: str, end_time: str
    ) -> QueryResult:
        ...
        if not process_field or not event_code:
            raise ValueError(f"Schema '{self.schema.schema_id}' does not support network_connection")
...
| LIMIT {self.max_results}'''
    ...
    def build_file_operations_query(self, process_name: str, host: str, start_time: str, end_time: str) -> QueryResult:
        ...
        if not process_field or not event_code:
            raise ValueError(f"Schema '{self.schema.schema_id}' does not support file_create")
...
| LIMIT {self.max_results}'''
    ...
    def build_registry_operations_query(
        ...
        if not process_field or not event_code:
            raise ValueError(f"Schema '{self.schema.schema_id}' does not support registry events")
...
| LIMIT {self.max_results}'''
    ...
    def build_process_access_query(
        ...
        if not source_field or not target_field or not event_code:
            raise ValueError(f"Schema '{self.schema.schema_id}' does not support process_access")
...
| LIMIT {self.max_results}'''
    ...
    def build_remote_thread_query(
        ...
        if not source_field or not target_field or not event_code:
            raise ValueError(f"Schema '{self.schema.schema_id}' does not support remote_thread")
...
| LIMIT {self.max_results}'''
    ...
    def build_dns_query(self, process_name: str, host: str, start_time: str, end_time: str) -> QueryResult:
        ...
        if not process_field or not event_code:
            raise ValueError(f"Schema '{self.schema.schema_id}' does not support dns_query")
...
| LIMIT {self.max_results}'''
    ...
    def build_host_activity_query(
        self, host: str, start_time: str, end_time: str, event_codes: list[str] | None = None
    ) -> QueryResult:
...
| LIMIT {self.max_results}"""
    ...
    def build_event_code_condition(self, event_code: str, use_alternatives: bool = False) -> str:
    ...
    def build_event_codes_in_condition(self, event_codes: list[str], use_alternatives: bool = False) -> str:
